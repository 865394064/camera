...
public class ZoomManager extends ViewManager implements CameraActivity.OnFullScreenChangedListener,
    ...
    private int mZoomRatioIndex = 0;
    private CameraActivity mContext;
    private long mCurrentTime = 0;
    private long mArcCurrentTime = 0;
    ...
    //begin: added by wangyouyou
    private int[] mAvailableRates = {100, 114, 132, 151, 174, 200, 229, 263, 303, 348, 400};
    public void setPerformZoom(int item) {
	if (true) {
            getContext().startAsyncZoom(item);
            int newRatio = mAvailableRates[item];
        ...
        if (true) {
            float zoomRation = ((float) mLastZoomRatio) / RATIO_FACTOR_RATE;
               /*getContext().getCameraAppUI().showInfo(
                    "x" + String.format(Locale.ENGLISH, PATTERN, (1 + (zoomRation - 1)*3)));*/
            mContext.setZoomRateBtnTextFormZoomManage(String.format(Locale.ENGLISH, PATTERN, (1 + (zoomRation - 1)*3))+"x");//added by wangyouyou
        }
    }

    public void computerPerformZoom(float dx, float totalX, boolean flag) {
		if(flag) {
			mZoomRatioIndex += (int)((totalX - dx) / 140);
		} else {
			mZoomRatioIndex += (int)((totalX - dx) / 70);
		}
		
		//android.util.Log.d("wyy", "totalX - dx = "+(totalX - dx));
	     if(mZoomRatioIndex > 10) {
			mZoomRatioIndex = 10;
	     } else if(mZoomRatioIndex < 0) {
			mZoomRatioIndex = 0;
	     }
	     setPerformZoom(mZoomRatioIndex);
    }

    public int getLastZoomRatio() {
	return mLastZoomRatio;
    }

    public int getLastZoomRatioIndex() {
	return mZoomRatioIndex;
    }

    public String getZoomRationString() {
		float zoomRation = ((float) mLastZoomRatio) / RATIO_FACTOR_RATE;
		return (1 + (zoomRation - 1)*3)+"x";
	}
    //end: added by wangyouyou
    ...
    private void performZoom(int zoomIndex, boolean userAction) {
        ...
        if (userAction) {
            ...
            //getContext().getCameraAppUI().showInfo(
               ...
               //getContext().getCameraAppUI().showInfo(
                   // "x" + String.format(Locale.ENGLISH, PATTERN, (1 + (zoomRation - 1)*3)));//del by wangyouyou
		 mContext.setZoomRateBtnTextFormZoomManage(String.format(Locale.ENGLISH, PATTERN, (1 + (zoomRation - 1)*3))+"x");//added by wangyouyou
        }
    }
    ...
    private class MyListener implements Listener {
        ...
        public boolean onScroll(float dx, float dy, float totalX, float totalY) {
	     ...
	     //begin: added by wangyouyou
	     //android.util.Log.d("wyy", "onScroll x = "+(dx - totalX));
	     if(!mContext.getZoomRateArcViewVisibleState() && !mContext.getScrollWhenZoomViewFlag()) {
			...
			mZoomRatioIndex += (int)((dx - totalX) / 210);
			}
		     if(mZoomRatioIndex > 10) {
				mZoomRatioIndex = 10;
		     } else if(mZoomRatioIndex < 0) {
		     ...
		     performZoom(mZoomRatioIndex, true);
		    mContext.updateZoomRateArcView(mZoomRatioIndex);
	     }
	     //end: added by wangyouyou
        ...
        public boolean onScale(float focusX, float focusY, float scale) {
            ...
            //performZoom(zoomIndex, true);//del by wangyouyou
