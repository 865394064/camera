...
public class SettingManager extends ViewManager implements View.OnClickListener,
    ...
    public static RotateImageView mIndicator;
    public static PickerButton mCameraPicker;//add by scq
    ...
    public SettingManager(CameraActivity context) {
		...
		filter.addAction("iphone_color_filter");
    ...
    protected View getView() {
        ...
        mIndicator = (RotateImageView)getContext().getIndicator();//add by scq
        mCameraPicker = (PickerButton) view.findViewById(R.id.setting_indicator); // added by scq
        return view;
    }
    ...
    public boolean collapse(boolean force) {
        ...
        if (mShowingContainer /*&& mAdapter != null*/) {
            //if (!mAdapter.collapse(force)) {   
                //hideSetting();
                hideFilterSetting();//added by wangyouyou
            //}
            collapsechild = true;
        }
	...
	/*End : add by scq for 20160920*/

	public boolean hideFilterSetting(){
	       ShutterManager.mPhotoShutter.setVisibility(View.VISIBLE);
		...
		getContext().runOnUiThread(new Runnable()
	      {
		        public void run()
		        {
		            ...
		            getContext().getColorEffectManager().hide();
	...
	public boolean expendChild()
  {
      ...
      getContext().runOnUiThread(new Runnable()
      {
        ...
        {      
                ...
                getContext().getColorEffectManager().show();
        }
      }); 
    ShutterManager.mPhotoShutter.setVisibility(View.GONE);
    mShowingContainer=  true;
    return mShowingContainer;
  }

   public static void showOrHideIndicator(boolean flag){
	...
	if(flag){
		...
		mIndicator.setImageResource(Color.BLACK);
		}
   }
  //end added by wangyouyou
    ...
    public void onStereoCameraSettingChanged(SettingListLayout settingList, ListPreference preference, int index, boolean showing) {
        ...
        if (getContext().getCurrentMode() == ModePicker.MODE_STEREO_CAMERA || (getContext().getCurrentMode() != ModePicker.MODE_STEREO_CAMERA && index == 2)) {
            refresh();
            return;
        }
        if (mShowingContainer && mAdapter != null) {
            if (!mAdapter.collapse(true)) {
                if (mShowingContainer && mSettingLayout != null) {
                    ...
                    getContext().getCameraAppUI().restoreViewState();
                    getContext().setSwipingEnabled(true);
                    mIndicator.setImageResource(R.drawable.ic_setting_normal_iphone);//modified by wangyouyou from R.drawable.ic_setting_normal
        ...
        if (getContext().isFullScreen()) {
                ...
                getContext().setSwipingEnabled(false);
                mIndicator.setImageResource(R.drawable.ic_setting_focus_iphone);//modified by wangyouyou from R.drawable.ic_setting_focus
    ...
    public void showSetting() {
        ...
        if (getContext().isFullScreen()) {
		  ...
		  expendChild();
                ...
                if(mIndicator != null)mIndicator.setImageResource(R.drawable.ic_setting_focus_iphone);//modified by wangyouyou from R.drawable.ic_setting_focus
            //}
            //setChildrenClickable(true);
        }
    }
    ...
    public void hideSetting() {
        ...
        if (mShowingContainer && mSettingLayout != null) {
            ...
            getContext().setSwipingEnabled(true);
            mListener.onSettingContainerShowing(mShowingContainer);
            mIndicator.setImageResource(R.drawable.ic_setting_normal_iphone);//modified by wangyouyou from ic_setting_normal
