...
import java.io.File;
import java.io.ByteArrayOutputStream;//added by xss for ForceTouch
import android.graphics.Bitmap;//added by xss for ForceTouch
...
public class ThumbnailViewManager extends ViewManager implements OnClickListener,
    ...
    public void onClick(View v) {
        if (getContext().isFullScreen() && getContext().isCameraIdle() && mThumbnail != null) {
	     getContext().setRecordingTimeShow(false);//add by scq
    ...
    public void updateThumbnailView() {
        ...
        if (mThumbnailView != null) {
            if (super.isShowing()) {
			 ...
			 it.setAction("get_thumbnail_photo");
			 it.putExtra("LasterThumbnail",getBytes(mThumbnail.getBitmap()));
...
public  byte[] getBytes(Bitmap bitmap){  
   ...
   /*End:added by xss for ForceTouch*/ 
    ...
    private void getLastThumbnail() {
        ...
        mLoadThumbnailTask = new LoadThumbnailTask(!FeatureSwitcher.isSmartBookEnabled()).execute();
    }
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_UPDATE_THUMBNAIL:
                if (!mCurrentSaveRequest.isContinuousRequest()) {
                    ...
                    if ((System.currentTimeMillis() - mCshotThumBegin) >= mCameraAnimation.SLIDE_DURATION) {
