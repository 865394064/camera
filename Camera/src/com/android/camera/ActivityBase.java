 ...
 */
public abstract class ActivityBase extends AbstractGalleryActivity implements
    ...
    // default value set to false to notice 
    // onAfterFullScreenChange when lunch Camera AP first time,
    // to set gallery surface view's alphpa to 0.0f
    protected boolean mShowCameraAppView = false;
    // default value set to false, sync with mShowCamerAppView, 
    // only when onFullScreenChange can change this value
    // let isFullScreen() gets right value
    // ex. onResume --> onPause occurs quickly, isFullScreen will get false
    ...
    protected void onPause() {
        Log.d("hjz","onPause");
    if(!CameraActivity.isFilterEditBtn)
         mAppBridge.switchWithCaptureAnimation(-1);//added by wangyouyou
    else 
         CameraActivity.isFilterEditBtn = false;//added by wangyouyou
        mPaused = true;
        super.onPause();
    }
    @Override
    ...
    protected void createCameraScreenNail(boolean getPictures) {
        ...
        mAppBridge.setSwipingEnabled(true);
    }
    ...
    protected void updateCameraAppView() {
        ...
        if (mShowCameraAppView) {
            ...
            mCameraAppView.animate().setDuration(CAMERA_APP_VIEW_TOGGLE_TIME).withLayer().alpha(1)
                    .withEndAction(null);
        } else {
            if (mHideCameraAppView == null) {
                mCameraAppView.setVisibility(View.INVISIBLE);
            } else {
                mCameraAppView.animate().setDuration(CAMERA_APP_VIEW_TOGGLE_TIME).withLayer().alpha(0)
                .withEndAction(mHideCameraAppView);
            }
        }
    }
    ...
    protected void updateCameraAppViewIfNeed() {
        ...
        // expand surface view size for onPuase has shrink surface view'size
        if (mShowCameraAppView && getPreviewSurfaceView() != null) {
            getPreviewSurfaceView().expand();
        }
    }
    ...
    private void onFullScreenChanged(boolean full, int type) {
        ...
        boolean needUpdateSurfaceView = (type == PhotoView.Listener.FULLSCREEN_TYPE_INTO_GALLERY_END)
                || (type == PhotoView.Listener.FULLSCREEN_TYPE_INTO_CAMERA_BEGIN);
        Log.i(TAG, "onFullScreenChanged(" + full + ", " + type + ") mShowCameraAppView="
                + mShowCameraAppView + ", mPaused=" + mPaused + ", isFinishing()=" + isFinishing()
                + ", scaleAnimation=" + scaleAnimation
                + " needUpdateSurfaceView = " + needUpdateSurfaceView
                + " getPreviewSurfaceView = " + getPreviewSurfaceView());
        if (getPreviewSurfaceView() != null && needUpdateSurfaceView) {
            if (full) {
                getPreviewSurfaceView().expand();
            } else {
                getPreviewSurfaceView().shrink();
            }
        }
    ...
    public boolean isActivityOnpause() {
        Log.i("hjz", "isActivityOnpause , mpaused = " + mPaused);
        return mPaused;
    }
    ...
    public void onLayoutChange(View v, int left, int top, int right, int bottom, int oldLeft,
        ...
        MMProfileManager.triggerProfileLayoutChange("onLayoutChange(left=" + left + ", top=" + top
        ...
        MMProfileManager.triggerProfileLayoutChange("call mAppBridge.setCameraRelativeFrame("
                + frame.toString() + ")");
        mAppBridge.setCameraRelativeFrame(frame);
        if (!frame.equals(mOldCameraRelativeFrame)) {
             mCameraScreenNail.setOnLayoutChanged(true);
            // when RelativeFrame changed , tell CameraScreenNail set New size
            // to SurfaceTextureScreenNail
        ...
        Log.d(TAG, "onLayoutChange() frame=" + frame + ", mOldCameraRelativeFrame="
                + mOldCameraRelativeFrame);
    ...
    public Rect getCameraRelativeFrame() {
        return mOldCameraRelativeFrame;
    }
    ...
    private boolean onTouchScreen(int x, int y, View area, int index) {
        ...
        int[] relativeLocation = Util.getRelativeLocation((View) getGLRoot(), area);
    ...
    protected void onPreviewSizeChanged(int width, int height) {
    ...
    class MyAppBridge extends AppBridge implements CameraScreenNail.Listener {
        ...
        // This is used to notify that the screen nail will be drawn in full
        // screen
        ...
        public void onStateChanged(int state) {
            ActivityBase.this.onStateChanged(state);
        }
        
        @Override
        public void onPreviewSizeChanged(int width, int height) {
            ActivityBase.this.onPreviewSizeChanged(width, height);
        }
...
//        }
        ...
        public void addSecureAlbumItem(boolean isVideo, int id) {
            if (mServer != null)
                mServer.addSecureAlbumItem(isVideo, id);
        }
        
        private void setCameraRelativeFrame(Rect frame) {
            if (mServer != null) {
                mServer.setCameraRelativeFrame(frame);
            }
        }
        ...
        private void setSwipingEnabled(boolean enabled) {
            if (mServer != null) {
                mServer.setSwipingEnabled(enabled);
            }
        }
