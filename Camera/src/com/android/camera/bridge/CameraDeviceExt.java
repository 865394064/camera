...
import com.android.camera.manager.ModePicker;//added by wangyouyou
import java.util.ArrayList;//added by wangyouyou
...
public class CameraDeviceExt implements ICameraDeviceExt {
    ...
    private static Activity mActivity;//added by wangyouyou
    private static ParametersExt mParametersExt;//added by wangyouyou
    ...
    public static String previewSize = "1.3333";//added by wangyouyou
    ...
    public static void setDisplaySize(String size){  
	previewSize = size;
	SettingUtils.setPreviewSize(mActivity, mParametersExt, previewSize);

	
	if(previewSize.equals("1.7778")){
		List<String> supportedSizes = new ArrayList<String>();
		supportedSizes.add("1280x720");
		supportedSizes.add("2560x1440");
		Point ps = SettingUtils.getSize(supportedSizes.get(previewSize.equals("1.0") ? 0 : 1));
		mParametersExt.setPictureSize(ps.x, ps.y);
	}else{
		List<String> supportedSizes = SettingUtils.buildSupportedPictureSize(mActivity,
		    mParametersExt, previewSize);
		
		if("1.0".equals(previewSize)) {   
			Point ps = SettingUtils.getSize(supportedSizes.get(0));
			mParametersExt.setPictureSize(ps.x, ps.y);
		} else if ("1.3333".equals(previewSize)) {
			Point ps = SettingUtils.getSize(supportedSizes.get(supportedSizes.size()-1));
			mParametersExt.setPictureSize(ps.x, ps.y);
		}	
	}
	   
	//android.util.Log.i("yuhuizhong","supportedSizes.get "+supportedSizes.get(0)+"  "+supportedSizes.get(1));
        
    }

  public static void setDisplaySizeSwitchCamera(String size){  
	previewSize = size;
	SettingUtils.setPreviewSize(mActivity, mParametersExt, previewSize);
    }
    ...
    public void setPreviewSize() {   
        ...
        if (pictureRatio == null) {
            List<String> supportedRatios = SettingUtils.buildPreviewRatios(mActivity, mParametersExt);
            if (supportedRatios != null && supportedRatios.size() > 0) {    
                SharedPreferences.Editor editor = mPreferences.edit();
                String ratioString = supportedRatios.get(0);//mody by xss from supportedRatios.size()-1 to 0
	 ...
	 //begin: added by wangyouyou
	 int supportedRatiosId = 0;
	 if(ModePicker.mCurrentModeId == ModePicker.MODE_FACE_BEAUTY){
	 	supportedRatiosId = 1;
	 }else if(ModePicker.mCurrentModeId == ModePicker.MODE_PANORAMA ||ModePicker.mCurrentModeId == ModePicker.MODE_LIVE_PHOTO ||
	 	ModePicker.mCurrentModeId == ModePicker.MODE_MOTION_TRACK || ModePicker.mCurrentModeId == ModePicker.MODE_MAV ){
	 	supportedRatiosId = 2;
	 }else {
	 	supportedRatiosId = 0;  
	 }
	 if(true){
		List<String> supportedRatios = SettingUtils.buildPreviewRatios(mActivity, mParametersExt);
		  SharedPreferences.Editor editor = mPreferences.edit();
                String ratioString = supportedRatios.get(supportedRatiosId);
                editor.putString(SettingConstants.KEY_PICTURE_RATIO, ratioString);
                editor.apply();
                pictureRatio = ratioString;
	 }
    ...
    public boolean isSceneModeChanged() {
        ...
        Log.d(TAG, "[isSceneModeChanged] curScene:" + curScene + ",mLastSceneMode:" + mLastSceneMode);
        return isChanged;
    }
    ...
    public boolean isPictureSizeChanged() {
        ...
        Log.d(TAG, "[isPictureSizeChanged] size:" + curPictureSize + ",oldsize:" + mLastPictureSize);
        return isChanged;
    }
