...
import android.widget.FrameLayout.LayoutParams;//added by wangyouyou
import com.android.camera.bridge.CameraDeviceExt;//added by wangyouyou
...
public class CameraDeviceCtrl implements SurfaceHolder.Callback {
    ...
    public static int changeCameraId = -1;//added by wangyouyou
    ...
    private static CameraActivity totalCameraActivity;//added by wangyouyou
    private final ComboPreferences mPreferences;
    private final MainHandler mMainHandler;
    
    private final CameraAppUiImpl mCameraAppUi;
    ...
    public CameraDeviceCtrl(CameraActivity activity, ComboPreferences preferences,
            CameraAppUiImpl cameraAppUi) {
        mCameraActivity = activity;
	 totalCameraActivity = activity;//added by wangyouyou
    ...
    public void setCameraScreenNail(CameraScreenNail cameraScreenNail) {
        Log.d(TAG, "[setCameraScreenNail] camerstate:" + getCameraState());
        mCameraScreenNail = cameraScreenNail;
    ...
    public void onPause() {
        ...
        // only in camera, show preview cover
        if (mCameraActivity.isFullScreen()) {
            // shrink surface view to let exit animation quickly
    ...
    public void switchCamera(int cameraId) {
        ...
        changeCameraId = cameraId;
	 try{
		 if(cameraId == 1){
			ModePicker.appearView();
			mCameraActivity.setMCameraId(false);
			mCameraActivity.hideAndShowPanoramaLayer(true);
			mCameraActivity.setCameraIdForPanoramaLayer(false);
			mCameraActivity.showOrHideZoomRateViewAndArcView(false);
		 }else{
			ModePicker.disappearView();      
			mCameraActivity.setMCameraId(true);
			//mCameraActivity.hideAndShowPanoramaLayer(true);
			mCameraActivity.setCameraIdForPanoramaLayer(true);
			mCameraActivity.showOrHideZoomRateViewAndArcView(true);
		 }
	 }catch (Exception e) {
    ...
    public void openStereoCamera(int cameraId) {
    ...
    public void onAfterFullScreeChanged(final boolean full) {
        if (full) {
            turnOnWhenShown();
        } else {
            turnOffWhenHide();
    ...
    public void onModeChanged(boolean isNeedRestart) {
        ...
        if (isCameraOpened()) {
            ...
            applyParameters(isNeedRestart, isNeedRestart);
        }
    }
   ...
   //end added by wangyouyou
    ...
    public void applyParameters(boolean isNeedRestart) {
        if (isCameraOpened()) {
            applyParameters(isNeedRestart, false);
        } else {
            Log.d(TAG, "[applyParameters] isNeedRestart:" + isNeedRestart + ",cameraState:"
                    + mCameraState, new Throwable());
        }
    }
    
    public void applyParameters(boolean isNeedRestart, boolean isShowRemaining) {
        Log.d(TAG, "[applyParameters] isNeedRestart:" + isNeedRestart + ",isShowRemaining:"
                + isShowRemaining);
        ...
        final boolean needRestart = isZsdChanged || isPreviewSizeChanged || isNeedRestart || isHdrChanged;
        if (needRestart) {
        ...
        if ((isPictureSizeChanged || isPreviewSizeChanged) || isShowRemaining || isVideoBitRateChanged
                || isAudioBieRateChanged) {
    ...
    //begin:added by wangyouyou
    /*public static void setTotalViewBottomDown(){
	FrameLayout surfaceViewRoot = (FrameLayout) totalCameraActivity.findViewById(R.id.camera_surfaceview_root);
	FrameLayout.LayoutParams mParams = (LayoutParams) surfaceViewRoot.getLayoutParams();
	mParams.bottomMargin = 0;
	surfaceViewRoot.setLayoutParams(mParams);
    }

    public static void setTotalViewBottomUp(){
	FrameLayout surfaceViewRoot = (FrameLayout) totalCameraActivity.findViewById(R.id.camera_surfaceview_root);
	FrameLayout.LayoutParams mParams = (LayoutParams) surfaceViewRoot.getLayoutParams();
    ...
    public void releaseSurfaceTexture(boolean needReleaseExtTexture) {
        ...
        mCameraScreenNail.releaseSurfaceTexture(needReleaseExtTexture);
    ...
    public void setDisplayOrientation() {
        ...
        if (mCameraActivity.getGLRoot() != null) {
            mCameraActivity.getGLRoot().requestLayoutContentPane();
        }
        if (mModuleManager != null) {
    ...
    private void prepareParameter(boolean needRestart, boolean isPreviewRatioChanged,
        ...
        if (needRestart) {
            // TODO why care mIsSwitchCamera?
            if (mIsSwitchCamera && isPreviewRatioChanged && mSurfaceTexture != null) {
    ...
    private void turnOffWhenHide() {
        Log.d(TAG, "[turnOffWhenHide] mCurCameraDevice:" + mCurCameraDevice +  ",mFocusManager:" + mFocusManager);
        if (mCurCameraDevice != mDummyCameraDevice && mFocusManager != null) {
            mCurCameraDevice.getParametersExt().setFlashMode(Parameters.FLASH_MODE_OFF);
            
            // when slip from Camera to Gallery, stop continous auto focus.
            if (Parameters.FOCUS_MODE_INFINITY == mFocusManager.getFocusMode()) {
                mIsNeedResetFocus = false;
            } else {
                mIsNeedResetFocus = true;
            }
            Log.d(TAG, "[turnOffWhenHide] mIsNeedResetFocus" + mIsNeedResetFocus);
            if (mIsNeedResetFocus) {
                if (SettingUtils.isSupported(Parameters.FOCUS_MODE_AUTO,
                        mCurCameraDevice.getSupportedFocusModes())) {
                    mFocusManager.overrideFocusMode(Parameters.FOCUS_MODE_AUTO);
                    mCurCameraDevice.setFocusMode(mFocusManager.getFocusMode());
                    // onAutoFocusMoving in progress, slide to gallery,should
                    // clear focus UI
    ...
    private void turnOnWhenShown() {
		...
		mCameraActivity.setCurrentFlashMode(currentFlashMode);//added by wangyouyou
	    String currentHdrMode = mISettingCtrl.getSettingValue(SettingConstants.KEY_HDR);//added by wangyouyou
	    mCameraActivity.setCurrentHdrMode(currentHdrMode);//added by wangyouyou
    ...
    private void switchPreview(boolean isDeviceUseSurfaceView, boolean isDisplayUseSurfaceView,
        ...
        // notify preview size changed
        // Size size = mParameters.getPreviewSize();
        Size size = mCurCameraDevice.getPreviewSize();
        int oldWidth = mCameraScreenNail.getTextureWidth();
        int oldHeight = mCameraScreenNail.getTextureHeight();
        updatePreviewBufferSize(oldWidth, oldHeight, size);
        ...
        if (isDeviceUseSurfaceView) {
            ...
            mCameraScreenNail.releaseSurfaceTexture(false);
    ...
    private void updatePreviewBufferSize(int oldWidth, int oldHeight, Size size) {
        // int cameraOrientation = Util.getDisplayOrientation(0, mCameraId);
        ...
        if (size != null) {
            ...
            if (oldWidth != width || oldHeight != height) {
                if (mCameraScreenNail.setScreenNailSize(width, height)) {
                    ...
                    mCameraActivity.notifyScreenNailChanged();
                }
            }
            if (mModuleManager != null) {
                mModuleManager.onPreviewBufferSizeChanged(width, height);
            }
            boolean isNeedFullScreenPreview = Math.abs((((double) Math.max(width, height) / Math
                    .min(width, height)) - SettingUtils.getFullScreenRatio())) < Util.ASPECT_TOLERANCE;
            mCameraScreenNail.enableAspectRatioClamping(isNeedFullScreenPreview);
            Log.i(TAG, "updateCameraScreenNailSize(" + oldWidth + ", " + oldHeight + ", "
                    + SettingUtils.buildSize(size) + ")" + " isNeedFullScreenPreview = "
                    + isNeedFullScreenPreview);
        }
    }
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_CAMERA_PREVIEW_DONE:
                ...
                if (mCameraActivity.isVideoMode()
                        || mCameraActivity.getCurrentMode() == ModePicker.MODE_LIVE_PHOTO) {
                    detachSurfaceViewLayout();
                }
                break;
    ...
    private class CameraStartUpThread extends Thread {
        ...
        private void prepareMockCamera() {
            ...
            if (null != mCameraDevice.getCamera() && FrameworksClassFactory.isMockCamera()) {
                // M: set context to create mock images
                mCameraDevice.getCamera().setContext(mCameraActivity);
            }
        }
