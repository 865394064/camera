...
import android.content.pm.ActivityInfo;//added by wangyouyou
import com.android.camera.ui.PickerButton;//add by scq
import com.android.camera.ui.RotateImageView;// added by scq
import android.view.animation.AlphaAnimation;// added by scq
import android.view.animation.Animation;// added by scq
import android.view.animation.LinearInterpolator;//add by scq

public class CameraAppUiImpl implements ICameraAppUi,OnFullScreenChangedListener{
    ...
    private static CameraActivity staticCameraActivity;//added by wangyouyou
    ...
    private HashMap<CommonUiType, ICameraView> mCameraViewArray = new HashMap<CommonUiType, ICameraView>();
    ...
    public CameraAppUiImpl(CameraActivity context) {
	 ...
	 staticCameraActivity = context;//added by wangyouyou
	...
	//begin: added by wangyouyou
	public ZoomManager getMZoomManager() {
		return mZoomManager;
	}
	//end: added by wangyouyou
    ...
    public void initializeCommonView() {
        ...
        if (mFaceBeautyEntryView != null) {
            mCameraViewArray.put(CommonUiType.FACE_BEAUTY_ENTRY, new CameraViewImpl(mFaceBeautyEntryView));
        }
    }
    ...
    public void initializeAfterPreview() {
        ...
        // We will show remaining first time and setViewState() do more thing
        // than we need.
        mShutterManager.show();
        mSettingManager.show();
        // mFacebeautyView.show();
        // For tablet
        if (FeatureSwitcher.isSubSettingEnabled()) {
    ...
    public void onFullScreenChanged(boolean full) {
        Log.i(TAG, "[onFullScreenChanged] full = " + full);
        // because current FaceBeauty Entry view not need put into the view root
        // so when goto gallery ,need hide the FaceBeautyEntryView itself
        // why need 0? Because int FaceBeautyEntryView, 0 means
        // onFullScreenChanged;

        mIsInCameraPreview = full;
        if (mFaceBeautyEntryView != null && !full) {
    ...
    public void setCameraId(int cameraId) {
        //del by scq :  mPickerManager.setCameraId(cameraId);
    }
    ...
    public void setSwipeEnabled(boolean enabled) {
        mCameraActivity.setSwipingEnabled(enabled);
    }
    
    @Override
    public void setZoomListener(ZoomListener listener) {
        mZoomManager.setZoomListener(listener);
    }
    ...
    public void setViewState(ViewState state) {
        ...
        } else if (FeatureSwitcher.isSubSettingEnabled() && mSubSettingManager.isShowSettingContainer()) {
    ...
    public void applayViewCallbacks() {
        mShutterManager.setShutterListener(mPhotoShutterListener, mVideoShutterListener, mCameraActivity
                .getCameraActor().getOkListener(), mCameraActivity.getCameraActor().getCancelListener());
    }
	...
	staticCameraActivity.findViewById(R.id.top_layer).setVisibility(View.GONE);
	staticCameraActivity.findViewById(R.id.bottom_layer).setVisibility(View.GONE);
    }

    public static void setViewVisible(){
	staticCameraActivity.findViewById(R.id.top_layer).setVisibility(View.VISIBLE);
	staticCameraActivity.findViewById(R.id.bottom_layer).setVisibility(View.VISIBLE);
    }
    //end:added by wangyouyou
    ...
    public void onConfigurationChanged() {
	 //mCameraActivity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);//added by wangyouyou
        for (ViewManager manager : mViewManagers) {      
            manager.reInflate();
        }
    ...
    private OnShutterButtonListener mPhotoShutterListener = new OnShutterButtonListener() {
        ...
        public void onShutterButtonClick(ShutterButton button) {
	     ...
	     setFlashShowOrHide();//add by scq
	     if(!mCameraActivity.getMCameraId()){//add by scq
		 	setHideOrShowFlashProactiveTextView(false);
			setHideOrShowFlashProactiveView(true);
			if(mModePicker.getCurrentMode() == mModePicker.MODE_FACE_BEAUTY){
				setHideOrShowView(false, false, false, true);
			}else{
				setHideOrShowView(false, false, false, false);
			}
	     }//add by scq
    ...
    private OnShutterButtonListener mVideoShutterListener = new OnShutterButtonListener() {
        ...
        public void onShutterButtonClick(final ShutterButton button) {
	     ...
	     if(!isVideo){//add by scq
		 	mCameraActivity.setRecordingTimeShow(isVideo);//add by scq
		 	isVideo = true;
	     }else{//add by scq
			mCameraActivity.setRecordingTimeShow(isVideo);//add by scq
		 	isVideo = false;
            ...
            if (mCameraActivity.isFullScreen()
                ...
                if (listener != null) {
                    listener.onShutterButtonClick(button);
                } else if (mModePicker.getModeIndex(mCameraActivity.getCameraActor().getMode()) != ModePicker.MODE_VIDEO) {
                    if (Storage.getLeftSpace() > 0) {
                        if (mModePicker.getModeIndex(mCameraActivity.getCameraActor().getMode()) == ModePicker.MODE_PHOTO_PIP) {
                         // new video pip mode
    ...
    private int getModePickerMode(CameraModeType mode) {
        ...
        case EXT_MODE_MOTION_TRACK:
            return ModePicker.MODE_MOTION_TRACK;
        
        case EXT_MODE_LIVE_PHOTO:
	...
	mSettingManager.onColorEffect(isShow);
    }

    public RotateImageView getIndicator(){
	return mPickerManager.getIndicator();
    }
	
    public PickerButton getCameraPicker(){
	return mSettingManager.getCameraPicker();
    }

    public void setHideOrShowFlashView(boolean isShow){
	mPickerManager.setHideOrShowFlashView(isShow);
    }
	
    public void setHideOrShowHdrView(boolean isShow){
        android.util.Log.d("kay15", "CameraAppUiImpl P1644 hideOrShowCameraHdrIcon: isShow:" + isShow);
	    mPickerManager.setHideOrShowHdrView(isShow);
    }    

    public void setHideOrShowCameraView(boolean isShow){
	mPickerManager.setHideOrShowCameraView(isShow);
    }    

    public void setHideOrShowTimeView(boolean isShow){
	mPickerManager.setHideOrShowTimeView(isShow);
    }
    public void setHideOrShowFlashProactiveView(boolean isShow){
	mPickerManager.setHideOrShowFlashProactiveView(isShow);
    }
	
    public void setHideOrShowFlashProactiveTextView(boolean isShow){
	mPickerManager.setHideOrShowFlashText(isShow);
    }
    public void setHideOrShowCameraPicker(boolean isShow){
	mPickerManager.setHideOrShowCameraPicker(isShow);
    }
    public void setHideOrShowView(boolean hdrFlag, boolean cameraFlag, boolean timeFlag, boolean dynamicFlag){
	mPickerManager.setHideOrShowView(hdrFlag, cameraFlag, timeFlag, dynamicFlag);
    }
    /*End : add by shenchengqi 20160920*/
    /*Start : add by shenchengqi 20161021*/
    private void setFlashShowOrHide(){
            //Log.i("Alinscq", "mCameraActivity.getMCameraId() = " + mCameraActivity.getMCameraId());
	     if(mPickerManager.getFlashProactiveNumber() == 2){
			return;
	     }
	     if(mCameraActivity.getMCameraId()){
		 	hideFlash(mCameraActivity.mFlashBackgroundView);
    		       stopFlick(mCameraActivity.mFlashView);
		 	return;
	     }
		 
	     showFlash(mCameraActivity.mFlashBackgroundView);
            startFlick(mCameraActivity.mFlashView);
