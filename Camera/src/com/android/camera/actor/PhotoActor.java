...
public class PhotoActor extends CameraActor implements FocusManager.Listener,
        OnShutterButtonListener, CameraActivity.OnFullScreenChangedListener {
    ...
    private android.hardware.Camera.PreviewCallback mCb;//added by wangyouyou for pre effect
    ...
    public PhotoActor(CameraActivity context, ModuleManager moduleManager, int mode) {
		...
		/*Begin added by wangyouyou for pre effect */
	this.mCb = new android.hardware.Camera.PreviewCallback()
      {
        public void onPreviewFrame(byte[] paramAnonymousArrayOfByte, android.hardware.Camera paramAnonymousCamera)
        {
		
	         if (PhotoActor.this.mPl != null){
	            for (preViewListner localpreViewListner : PhotoActor.this.mPl){
	              if (localpreViewListner != null)
	               localpreViewListner.startNPreView(paramAnonymousArrayOfByte,null);
	            	}
	         }
        }
      };
	  /*End added by wangyouyou for pre effect*/
    }
	/*Begin added by wangyouyou for pre effect*/
	public android.hardware.Camera.PreviewCallback getPreviewCallback()
	  {
	    return this.mCb;
	  }
  /*End added by wangyouyou pre effect*/
    ...
    public void onCameraParameterReady(boolean startPreview) {
        ...
        if (startPreview) {
            if (!mModuleManager.startPreview(true)||mCameraActivity.isStartFilterCamera) {//del by wangyouyou
                startPreview(true);
            }
        }
    ...
    public void stopPreview() {
        ...
        if (mCameraActivity.getCameraState() != CameraActivity.STATE_PREVIEW_STOPPED
                && (!mIsZSDEnabled || (mIsZSDEnabled && mCameraActivity.getCameraState() != CameraActivity.STATE_SNAPSHOT_IN_PROGRESS))) {
            // stop preview may not be called in main thread,we need to
            // synchronized "stopPreview" and "sFaceDetectionStarted = false"
            // Exception Case: touch focus between onCamearOpenDone and
            // startPreview Done,press home key to exit camera and then enter
            // immediately
            if(mCameraActivity.getColorEffectManager() != null)  {
     		mCameraActivity.getColorEffectManager().setCameraPreViewDone(false);
    ...
    public void onShutterButtonClick(ShutterButton button) {
        ...
        Log.d("hjz","click for Photograph");
        if("3000".equals(SystemProperties.get("persist.cenon.clockfor3s","1"))){

            mSelfTimerManager.setSelfTimerDuration("3000");
        }else if("10000".equals(SystemProperties.get("persist.cenon.clockfor3s","1"))){
            mSelfTimerManager.setSelfTimerDuration("10000");
        }else{
            mSelfTimerManager.setSelfTimerDuration("0");
        }
        if (!(CameraActivity.STATE_SWITCHING_CAMERA == cameraState
                || CameraActivity.STATE_PREVIEW_STOPPED == cameraState)) {
    ...
    public void onFullScreenChanged(boolean full) {
        ...
        mModuleManager.onFullScreenChanged(full);
    }
    ...
    public void release() {
        ...
        mContext.removeOnFullScreenChangedListener(this);
	...
	//if (!(this.mCamera.getCameraActor() instanceof PanoramaActor))
      mCameraActivity.getColorEffectManager().setCameraPreViewDone(mCameraActivity.getColorEffectManager().isShowing());
    ...
    private void restartPreview(boolean needStop) {
        ...
        mCameraActivity.setCameraState(CameraActivity.STATE_IDLE);
        mICameraAppUi.restoreViewState();
        mCameraActivity.setSwipingEnabled(true);
        startFaceDetection();
    }
    ...
    private CameraActivity.OnSingleTapUpListener mOnSingleTapListener = new CameraActivity.OnSingleTapUpListener() {
        ...
        public void onSingleTapUp(View view, int x, int y) {
            ...
            if (!focusManager.getFocusAreaSupported()) {
                Log.i(TAG, "[onSingleTapUp] getFocusAreaSupported is false,return.");
                return;
            }

            String focusMode = focusManager.getCurrentFocusMode(mCameraActivity);
            if (focusMode == null || Parameters.FOCUS_MODE_INFINITY.equals(focusMode)) {
                Log.i(TAG, "[onSingleTapUp] focusMode = " + focusMode);
                return;
            }
