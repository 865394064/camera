...
import com.android.camera.CameraActivity;//added by wangyouyou
public class VideoPreviewRule implements ISettingRule {
    ...
    private String mConditionSettingKey = SettingConstants.KEY_VIDEO;
	 private final String PICTURE_RATIO_16_9 = "1.7778";//added by wangyouyou
    ...
    public VideoPreviewRule(ICameraContext cameraContext, CameraModeType mode) {
        ...
        } else if (mMode == CameraModeType.EXT_MODE_LIVE_PHOTO) {
            mConditionSettingKey = SettingConstants.KEY_LIVE_PHOTO;
        }
    }
    ...
    public void execute() {
        ...
        if (index == -1) {
            ...
            SettingUtils.setPreviewSize(mActivity, mParameters, ratio);
			  ...
			  CameraDeviceExt.setDisplaySize(PICTURE_RATIO_16_9); //added by wangyouyou
			  ((CameraActivity)mActivity).setMyCurrentMode(2);//added by wangyouyou
            ...
            if (mMode == CameraModeType.EXT_MODE_LIVE_PHOTO) {
                ListPreference pref = mISettingCtrl.getListPreference(SettingConstants.KEY_PICTURE_RATIO);
                if (pref != null) {
                    pref.setEnabled(true);
                }
            }
			  ((CameraActivity)mActivity).setIsLeaveVideoPreviewFlag(true);//added by wangyouyou
			  ((CameraActivity)mActivity).setSurfaceViewLocation(true);//added by wangyouyou
        } else {
            setVideoPreviewSize();
			 CameraDeviceExt.setDisplaySize(PICTURE_RATIO_16_9); //added by wangyouyou
	    ((CameraActivity)mActivity).setVideoFlag(true);//added by wangyouyou
            mHasOverride = true;
        }
    ...
    private int getQuality() {
        ...
        int quality;
        // live photo quality do not get from settings other mode's quality will
        // get from settings
        if (mMode == CameraModeType.EXT_MODE_LIVE_PHOTO) {
            return getLivePhotoQuality();
        } else {
            if ("on".equals(mISettingCtrl.getSettingValue(SettingConstants.KEY_SLOW_MOTION)) && !mSwitchingPip) {
                ...
                mISettingCtrl.setSettingValue(SettingConstants.KEY_VIDEO_QUALITY, settingQuality, mICameraDeviceManager
                    .getCurrentCameraId());
            } else {
    ...
    private void getCameraDevice() {
        if (mICameraDeviceManager != null) {
            ...
            if (mTopICameraDevice != null) {
                mSwitchingPip = (mTopICameraDevice.getCameraId() == mICameraDeviceManager.getCurrentCameraId());
            } else {
                mSwitchingPip = false;
            }
            updateTopParameters();
    ...
    private int conditionSatisfied(String conditionValue) {
        ...
        Log.i(TAG, "[conditionSatisfied]limitation index:" + index + " conditionValue = " + conditionValue);
        return index;
    }
    ...
    private Point computeDesiredPreviewSize(CamcorderProfile profile, Parameters parameters) {
        ...
        if (parameters.getSupportedVideoSizes() == null) {// should be rechecked
            ...
            Size optimalSize = Util.getOptimalPreviewSize(mActivity, sizes,
                    (double) profile.videoFrameWidth / profile.videoFrameHeight, true, false); // don't
                                                                                               // change
                                                                                               // target
                                                                                               // ratio
            if (optimalSize != null) {
    ...
    private void reviseVideoCapability(Parameters parameters, CamcorderProfile profile) {
        Log.d(TAG, "[reviseVideoCapability()] begin profile.videoFrameRate="
    ...
    // live photo will select the quality which is the most close to the screen
    // size
    private int getLivePhotoQuality() {
        ...
        int quality;
        ListPreference videoqualityPreference = mISettingCtrl
                .getListPreference(SettingConstants.KEY_VIDEO_QUALITY);
        int minDiff = Integer.MIN_VALUE;
        int height = Math.min(point.x, point.y);
        int width = Math.max(point.x, point.y);
        Log.i(TAG, "[getLivePhotoQuality] live photo find width = " + width + " height = " + height);
        ...
        if (videoqualityPreference != null) {
            ...
            for (int i = 0; i < supportedVideoQualitys.length; i++) {
                ...
                CamcorderProfile profile = fetchProfile(tempQuality,
                ...
                if (Math.abs(aspectRatio - targetAspectRatio) <= Math.abs(minAspectio
                        - targetAspectRatio)) {
            ...
            for (int i = 0; i < supportedVideoQualitys.length; i++) {
                ...
                if (Math.abs(targetAspectRatio - aspectRatio) < Util.ASPECT_TOLERANCE) {
                    if (Math.abs(profile.videoFrameHeight - height) > minDiff) {
                        ...
                        Log.i(TAG,
                                "[getLivePhotoQuality] live photo find profile.videoFrameHeight = "
                                        + profile.videoFrameHeight + " height = " + height
                                        + " tempQuality = " + tempQuality + " minDiff = " + minDiff
        ...
        Log.i(TAG, "[getLivePhotoQuality] live photo find quality : " + resultQuality);
        return quality;
    }
    ...
    private void pipDenaliZSDRule(int index){
        ...
        int type = zsdSetting.getType();
         if (!MTK_CHIP_0321.equals(mICameraContext.getFeatureConfig().whichDeanliChip()) || mICameraContext.getFeatureConfig().isLowRamOptSupport()) {
             return;
         }
        if (index != -1) {
                        ...
                        if (pref != null && SettingUtils.isBuiltList(overrideValue)) {
                            ...
                            if (list.contains(prefValue)) {
                                if (!prefValue.equals(value)) {
                                    ...
                                    overrideValue = SettingUtils.buildEnableList(list.toArray(values),prefValue);
                                }
                                value = prefValue;
                            }
                        }
