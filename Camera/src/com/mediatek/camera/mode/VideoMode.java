...
public class VideoMode extends CameraMode implements MediaRecorder.OnErrorListener,
    ...
    private Handler mTimerHandler = new Handler(){  
            ...
            public void handleMessage(Message msg) {  
                mRecordingView.flashRecordIcon();
            }  
      };  
    //end: added by wangyouyou
    ...
    public VideoMode(ICameraContext cameraContext) {
        ...
        mVideoModeHelper = new VideoModeHelper(mActivity, mIModuleCtrl, mISettingCtrl);
		 wm = (WindowManager)mActivity.getSystemService(Context.WINDOW_SERVICE);//added by wangyouyou
    }
    ...
    public void onInfo(MediaRecorder mr, int what, int extra) {
        ...
        case MediaRecorder.MEDIA_RECORDER_INFO_CAMERA_RELEASE:
    ...
    protected boolean startVideoRecording() {
        ...
        if (!mIsTimeLapseEnable) {
            ...
            //mHandler.removeMessages(UPDATE_RECORD_TIME);//del by wangyouyou
        }
        keepScreenOn();
    ...
    protected void initializeNormalRecorder() {
        ...
        mMediaRecorder.setVideoSize(wm.getDefaultDisplay().getHeight(), wm.getDefaultDisplay().getWidth());//added by wangyouyou
        mMediaRecorder.setVideoEncoder(mProfile.videoCodec);
        MediaRecorderEx.setVideoBitOffSet(mMediaRecorder, 1, true); // TODO
        if (!mIsTimeLapseEnable && mIsRecordAudio) {
    ...
    protected boolean startNormalRecording() {
        ...
        try {
	  	...
	  	startTimer();//added by wangyouyou
        return isSuccess;
    }
	...
	((CameraActivity)mActivity).setVideoFlag(false);//added by wangyouyou
    ...
    protected void stopVideoOnPause() {
        ...
        if (mIsMediaRecorderRecording) {
            ...
            releaseMediaRecorder();
            boolean hasSaveRunnable = mHandler.hasCallbacks(mVideoSavedRunnable);
            Log.d(TAG, "[stopVideoOnPause()] has mVideoSavedRunnable = " + hasSaveRunnable);
            if (mIModuleCtrl.isNonePickIntent() && !isVideoProcessing() && !hasSaveRunnable) {
                backToLastTheseCase();
            }
        }
        if (videoSaving) {
    ...
    protected void addVideoToMediaStore() {
        Log.i(TAG, "[addVideoToMediaStore] mVideoFileDescriptor = " + mVideoFileDescriptor);
        if (mVideoFileDescriptor == null) {
    ...
    public final class JpegPictureCallback implements PictureCallback {
        ...
        public void onPictureTaken(byte[] jpegData, android.hardware.Camera camera) {
            ...
            String title = Util.createNameFormat(time,
                    ...
                    + ".jpg";
            mIFileSaver.savePhotoFile(jpegData, null, time, true, mIModuleCtrl.getLocation(), 0,
                    null);
            mHandler.sendEmptyMessage(UPDATE_SNAP_UI);
        }
    }
    ...
    protected void initializeShutterStatus() {
        ...
        if (CameraInfo.CAMERA_FACING_FRONT == mICameraDeviceManager
            ...
            mICameraAppUi.setPhotoShutterEnabled(mParameters.isVideoSnapshotSupported());
        }
    }
