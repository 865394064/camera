...
import com.android.camera.manager.ShutterManager;//added by wangyouyou
...
public class RecordingView extends CameraView implements OnClickListener {
    ...
    private ImageView mRedDotView;//added by wangyouyou
    ...
    public View getView() {
	 ...
	 mRedDotView = (ImageView) view.findViewById(R.id.recording_red_dot);//added by wangyouyou
    ...
    public void flashRecordIcon() {
		mRedDotView.setVisibility(flag ? View.INVISIBLE:View.VISIBLE);
    ...
    public void hide() {
        super.hide();
		ModePicker.isRecording = false;//added by wangyouyou
        setSizeProgress(0);// reset progress to 0
        //begin:added by wangyouyou
        if(CameraDeviceCtrl.changeCameraId == 1){
		ModePicker.setVideoMode(ModePicker.getVideoMode(), true);
	}else 
	      ModePicker.setVideoMode(ModePicker.getVideoMode(), false);
	 //end:added by wangyouyou
    }
    ...
    public void showTime(long millis, boolean showMillis) {
		ModePicker.isRecording = true;//added by wangyouyou
	 ShutterManager.setTemp();//added by wangyouyou
    ...
    private String formatTime(long millis, boolean showMillis) {
        ...
        if (showMillis) {
            ...
            if (hours > 0) {
                ...
                //text = String.format(Locale.ENGLISH, "%02d:%02d", minutes, seconds);//del by scq
                text = String.format(Locale.ENGLISH, "%02d:%02d:%02d", hours, minutes, seconds);//add by scq
            }
        }
