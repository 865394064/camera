...
public class SettingGenerator {
    ...
    private static final String[] SLOW_MOTION_SUPPORT_QUALIYS_STRING = new String[] {
            SLOW_MOTION_QUALITY_HD_120FPS, SLOW_MOTION_QUALITY_HD_180FPS,
            SLOW_MOTION_QUALITY_FHD_120FPS, SLOW_MOTION_QUALITY_VGA_120FPS,
            SLOW_MOTION_QUALITY_HD_60FPS, SLOW_MOTION_QUALITY_FHD_60FPS };
    ...
    public void updatePreferences() {
        ...
        if (preferences == null) {
            ...
            if (mICameraContext.getFeatureConfig().isVfbEnable()) {
                SettingItem fdSetting = getSettingItem(SettingConstants.ROW_SETTING_CAMERA_FACE_DETECT, mCameraId);
                if (fdSetting.getListPreference() != null) {
                    fdSetting.getListPreference().setValue(DEFAULT_ON);
                    fdSetting.setValue(DEFAULT_ON);
                    fdSetting.setDefaultValue(DEFAULT_ON);
                }
                preferences.get(SettingConstants.ROW_SETTING_CAMERA_FACE_DETECT).setValue(DEFAULT_ON);
            }
    ...
    public void restoreSetting(int cameraId) {
        ...
        if (mICameraContext.getFeatureConfig().isVfbEnable()) {
            SettingItem fdSetting = getSettingItem(SettingConstants.ROW_SETTING_CAMERA_FACE_DETECT, cameraId);
            if (fdSetting != null && fdSetting.getListPreference() != null) {
                fdSetting.getListPreference().setValue(DEFAULT_ON);
                fdSetting.setValue(DEFAULT_ON);
                fdSetting.setDefaultValue(DEFAULT_ON);
            }
    ...
    private boolean filterPreference(final ListPreference preference) {
        ...
        case SettingConstants.ROW_SETTING_PICTURE_SIZE:// camera special case
            ...
            List<String> supportedForRatio = SettingUtils.buildSupportedPictureSize(mContext,
                    parameters, pictureRatio);
            removePreference = filterDisabledOptions(preference, supportedForRatio, false,
                    settingId);
            break;

        case SettingConstants.ROW_SETTING_VOICE:
            if (!mICameraContext.getFeatureConfig().isVoiceUiSupport()) {
                removePreference = true;
            }
            break;
    ...
    private void updateSettingItem(String key, ListPreference item) {
        ...
        ICameraFeatureExt updateString =  ExtensionHelper.getCameraFeatureExtension(null);
    ...
    public boolean isParametersSupport(int quality, List<SlowMotionParam> slowMotionParam) {
        ...
        CamcorderProfile profile = FrameworksClassFactory.getMtkCamcorderProfile(currentCamera,
                quality);
